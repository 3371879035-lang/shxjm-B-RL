...
class RolloutPlanner:
    ...
    def _candidates(self, env):
        ...
        for ch,st in env.channels.items():
            if st.status!='discovered':
                continue
            if not any(ob.get('result')=='direction' for ob in st.observations):
                continue
            center=np.asarray(st.poly_center,dtype=float)
            travels=float(np.linalg.norm(center-env.pos))
            resolves.append((travels,ch))
        resolves.sort(key=lambda t:t[0])
        resolves=resolves[:self.max_clear_candidates]
        return points, scans, clears, resolves

    def _resolve_channel(self, env, ch):
        st=env.channels[int(ch)]
        if st.status!='discovered' or env.done:
            return
        first=None; deg=None
        for ob in st.observations:
            if ob.get('result')=='direction' and ob.get('svd_deg') is not None:
                first=np.asarray(ob['position'],dtype=float); deg=float(ob['svd_deg']); break
        if first is None:
            return
        P=st.poly if st.poly is not None and len(st.poly)>0 else None
        def measure_cb(q, ch=int(ch)):
            out=env.measure(q,ch,is_refine=True)
            return {'measure_result':out.get('measure_result'),'svd_deg':out.get('svd_deg')}
        def clear_cb(q, ch=int(ch)):
            out=env.clear(q,ch)
        ...
        try:
            ...
            for q in optical_fallback_points(first,deg):
                if env.channels[int(ch)].status=='cleared' or env.done:
                    break
                if env.clear(q,int(ch)).get('clear_result')=='success':
                    break
    ...
    def _run_candidate(self, env, candidate):
        ...
        if kind=='scan':
            ...
            _,ch,_,_,_=candidate
            self._resolve_channel(env2,int(ch))
        pol=G25OPolicy(self.variant,coverage=self.coverage)
        if not env2.done:
            out=pol.run(env2, resume=True)
        else:
    ...
    def run(self, env):
        ...
        while not env.done:
            ...
            base_out=base.run(base_env, resume=True) if not base_env.done else {'virtual_time_s':base_env.virtual_time}
            base_total=float(base_out['virtual_time_s'])
            points,scans,clears,resolves=self._candidates(env)
            candidates=[]
            ...
            for travel,ch in resolves:
                candidates.append(('resolve',ch,None,None,travel))
            ...
            if best is None:
                ...
                # 没有可行动作：回退基策略
                self.fallbacks += 1
                return base.run(env, resume=True)
            ...
            if kind=='scan':
                ...
                _,ch,_,_,_=best
                self._resolve_channel(env,int(ch))
