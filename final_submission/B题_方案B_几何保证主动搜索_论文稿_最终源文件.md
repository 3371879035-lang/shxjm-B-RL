...
在此基础上，本文进一步按S21保证覆盖+零绕行主动补测+顺路清除+双侧保底的机制实现了G21A原型，并在相同种子、相同源和相同误差场下与G25OR-S21做100对严格配对。零绕行补测的点位于覆盖点A到B的路段上，若预测能把可行域MEC半径缩到19.5 m以内或者预计收缩超过200 m，才追加一次最多6 s的测量；Q4每源最多1次，no_signal不删除可行域。结果是：G21A在均匀场景平均虚拟时间为7069.63 s，反而比G25OR-S21的6709.32 s慢5.37%；边缘场景为8951.08 s，比基线的9000.00 s仅快0.55%；全部100对均清除，但平均每局只有约8.6-11.9次补测，补测后直接获得清除证书的次数平均仅0.04-0.06次。诊断表明，方向型第二观测主要压缩横向不确定度，而双侧定位的主要行走成本来自首测距离区间，后者并没有被明显压缩，因此补测的6 s成本大于它节省的定位移动。G21A没有达到平均V/源下降大于5%、中位数下降大于3%、至少4类场景明显占优的淘汰门槛，所以停止G21X及S21路线扩展，最终方案仍冻结为G25OR-S25。
...
7. Q1可实现反例、Q2条件候选区与817.749 m连续最坏距离、Q3边界网格缺口和Q4局部凸包证书均已审计；Oracle上下界给出每源不超过7.75秒的知情差距范围。完整滚动版在修复后仍未优于G25OR，只能说明该实现无额外收益，不能说明调度空间已经饱和；G21A零绕行补测原型在100对均匀配对中反而慢5.37%，已按淘汰门槛停止。
