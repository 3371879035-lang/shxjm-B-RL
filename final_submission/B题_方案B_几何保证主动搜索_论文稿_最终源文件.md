...
修复零绕行坐标bug后，G21A在六类场景N=10..16矩阵中总体比G25OR-S21慢3.19%；官方问题4各100局全清，G21A聚合540.00 s/源，S21基线514.15 s/源，G21A慢5.54%（Mann-Whitney p=0.046），平均移动距离反而多约1957 m/局。补测未转化成净收益，G21A未达到淘汰门槛；停止G21A/G21X/S21扩展，最终方案仍冻结为G25OR-S25。进一步在真实官方模拟器上对问题4各跑100局：G21A全部100/100清除，聚合虚拟时间540.00 s/源；G25OR-S21基线100/100清除，聚合514.15 s/源；G21A慢5.54%（Mann-Whitney p=0.046，Welch p=0.066），平均移动距离高约1957 m/局。官方结果与本地矩阵一致，确认G21A-fixed相对S21没有净收益；因此停止G21A/G21X/S21扩展，最终方案仍冻结为G25OR-S25。
...
进一步在50个问题3、50个问题4场景上做五策略严格配对的Oracle Ladder。问题3各策略平均时间/源为：G25OR 332.05、O1 308.58、O2 288.14、O3 256.77、O0 142.83 s；问题4为：G25OR 613.97、O1 512.48、O2 528.91、O3 402.79、O0 144.32 s。五种策略均100%清除；O0通过N=1解析公式和全向/定向同位置一致性检查。分解得到：问题3/4空频道成本约23.47/101.49 s/源，定位成本约43.91/85.06 s/源，空间搜索成本O3-O0约113.94/258.47 s/源，总改进空间约189.22/469.65 s/源。最大结构瓶颈是未知源空间搜索，其次在问题4是空频道扫描；下一步应优先研究可证明的搜索覆盖、频道账本和发现顺序，而不是继续第二测点或零绕行补测。
...
7. Q1可实现反例、Q2条件候选区与817.749 m连续最坏距离、Q3边界网格缺口和Q4局部凸包证书均已审计；Oracle上下界给出每源不超过7.75秒的知情差距范围。完整滚动版在修复后仍未优于G25OR，只能说明该实现无额外收益，不能说明调度空间已经饱和；G21A零绕行补测原型在六类场景N=10..16的1260次运行矩阵中总体慢3.19%，官方问题4各100局中又慢5.54%，全部清除但未达淘汰门槛，已停止。进一步Oracle Ladder分解显示，问题3/4最大改进空间分别为约189/470 s/源，其中空间搜索占约114/258 s/源、空频道占约23/101 s/源、定位占约44/85 s/源，下一步应优先研究可证明的搜索覆盖与频道账本，而不是继续第二测点或零绕行补测。
