...
在此基础上，本文进一步按S21保证覆盖+零绕行主动补测+顺路清除+双侧保底的机制实现了G21A原型，并做六类场景、N=10..16、每类每组15组的严格配对矩阵，共630对、1260次策略运行。零绕行补测点位于覆盖点A到B的路段上，若预测能把可行域MEC半径缩到19.5 m以内或预计收缩超过200 m，才追加一次最多6 s的测量；Q4每源最多1次，no_signal不删除可行域。矩阵结果：全部630对均清除；G21A总体平均V/源比G25OR-S21高3.68%（配对t检验p≈3.7×10^-77）；六类场景分别高5.18%、5.52%、2.99%、4.13%、0.29%、5.07%，其中边缘、均匀、混合场景均明显更慢；平均每局补测约8-12次，补测后直接获得清除证书平均仅0.04-0.06次。诊断表明，方向型第二观测主要压缩横向不确定度，而双侧定位的主要行走成本来自首测距离区间，后者没有被明显压缩，因此补测的约6 s成本大于节省的定位移动。G21A没有达到平均V/源下降大于5%、中位数下降大于3%、至少4类场景明显占优的淘汰门槛，所以停止G21X及S21路线扩展，最终方案仍冻结为G25OR-S25。进一步在真实官方模拟器上对问题4各跑100局：G21A全部100/100清除，聚合虚拟时间540.00 s/源；G25OR-S21基线100/100清除，聚合514.15 s/源；G21A慢5.54%（Mann-Whitney p=0.046，Welch p=0.066），平均移动距离高约1957 m/局。官方结果与本地矩阵一致，确认G21A-fixed相对S21没有净收益；因此停止G21A/G21X/S21扩展，最终方案仍冻结为G25OR-S25。
...
7. Q1可实现反例、Q2条件候选区与817.749 m连续最坏距离、Q3边界网格缺口和Q4局部凸包证书均已审计；Oracle上下界给出每源不超过7.75秒的知情差距范围。完整滚动版在修复后仍未优于G25OR，只能说明该实现无额外收益，不能说明调度空间已经饱和；G21A零绕行补测原型在六类场景N=10..16的1260次运行矩阵中总体慢3.19%，官方问题4各100局中又慢5.54%，全部清除但未达淘汰门槛，已停止。
