...
进一步在同一批50对场景上做三臂因果拆分，逐场景满足 G25OR - O0 = (G25OR - C) + (C - B) + (B - O0)：A仅发现为130.51/329.90 s/源；B发现后接真值最优清除为268.85/468.18 s/源；C仅用真值选择合法覆盖/求解顺序为330.11/606.28 s/源。三项平均为顺序损失1.94/7.69、合法信息与证明成本61.27/138.10、搜索-清除整合损失126.02/323.86 s/源，合计189.22/469.65 s/源（问题3/问题4）。顺序损失只占总差距1.0%/1.6%，而搜索-清除串行结构占66.6%/69.0%，说明主要矛盾是固定全覆盖先发现后清除，而不是顺序细调；下一步应优先研究可证明的在线搜索-清除联合路线与空频道账本。

同一批50对场景的Clear-as-Search影子审计显示：首停点处平均仍有5.92/7.84个真实源未发现，但顺手测量仅发现0/10个；有效整航点删除0/0个、距离节省0/0 m、净节省-23.33/-81.54 s/源；即使删除首停点后全部覆盖任务，Q3仅25.02 s/源，低于30-40 Gate，Q4的109.11 s/源不可达。现有clear/定位停点不能替代搜索，B3式补测不应实现，搜索-清除合并需重新设计搜索感知路线。
