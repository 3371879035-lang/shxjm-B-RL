# 无线电干扰源快速自动定位与清除：几何保证搜索与强化学习调度

## 摘要

本文针对 2026 年高教社杯全国大学生数学建模竞赛 B 题，建立了一套以可证明发现覆盖、集合定位和有限步骤清除为硬保证，以强化学习调度为可选上层策略的机器狗自动定位清除方案。方案将模型分成两层：保证层负责覆盖证书、定位可行域、清除证书和光学保底；决策层在有限合法动作中选择下一步，不影响不漏源的完成保证。
...
在调度层，将任务建模为部分可观测半马尔可夫决策过程，采用96槽位宏动作和带动作屏蔽的PPO。奖励直接对齐虚拟总时间 r_t=-ΔT_t/100。由于本地与官方实验均显示PPO在全部决策中未偏离A0先验，本文将PPO定位为安全调度扩展，而将严格几何主动搜索 G25OR 作为最终主方案。G25OR 同时修复了首次示向三角形约0.233米的外包缺口，并使用最小包围圆作为清除证书。

官方模拟器实验方面，G25O、G25OR、G25O-R full 三个版本各完成问题3、问题4各100局演练，全部局数分别为100/100、100/100、100/100清除。G25OR 问题3、问题4平均定位清除时间分别为328.81秒/源和556.83秒/源，平均程序运行时间分别为1.462秒和3.548秒；相对更早的旧安全PPO分别降低约23.3%和50.4%，相对传统方法有效演练组分别降低约44.7%和49.9%。三版本之间两两差异均不显著（p>0.17），说明性能主要来自几何保证层而非PPO网络。

关键词：交会定位；半平面求交；最小包围圆；25点覆盖；双侧区间定位；动作屏蔽；PPO；光学保底
...
3. 决策层：A0确定性策略、带动作屏蔽PPO、安全门。该层只影响效率，不影响正确性。
...
总体技术路线如图1所示。正确性由三层保证：S3与S25保证发现覆盖，频道账本保证不遗漏，最小包围圆与双侧定位保证清除。效率由A0、动作屏蔽PPO和安全门组成的调度层优化，但调度层只改变动作顺序，不改变有限步骤完成路径。模型检验采用解析证明、数值采样、本地严格配对、官方演练、非参数检验与成本敏感性分析相结合：解析证明给出最坏界，20万级采样检验边界与漏检，相同种子与误差场的本地配对控制混杂因素，官方100局分布比较验证工程稳定性，Mann-Whitney检验避免把非配对差值写成确定增益。正式测试尚未执行，因此第10.7节的记录表暂不填写，待组委会给出正式案例后按冻结参数运行。
...
| 几何保证+RL调度 | 几何层保证覆盖与清除，RL只决定顺序 | 正确性可证明，效率可学习 | 需要两层接口设计 | 是 |

选择第三类，因为题目首要目标是确保全部源被清除，不能让完成率依赖奖励函数权重。保证层提供有限步骤完成路径，调度层只优化时间。
...
### 11.6 调度：为什么选带动作屏蔽PPO并保留A0

候选调度方法包括A0贪心、滚动时域规划、PPO、DQN、SAC和MCTS。A0简单可靠，但短视；滚动规划需要场景采样和较大在线计算；DQN对持续时间不同的宏动作折扣处理不自然；SAC适合连续动作，不适合本题的离散候选点选择；MCTS需要大量在线模拟。PPO适合96槽位离散宏动作，可通过动作屏蔽禁止明显无效动作，训练相对稳定。
...
两两差异均不显著（p>0.17）。因此选择G25OR：它修复了首次示向三角形的严格外包缺口，同时官方成绩与G25O、G25O-R full等价；G25O-R full的额外复杂度和程序运行时间没有换来统计显著的速度提升。
...
5. 官方G25O、G25OR、G25O-R full各100+100局全部清除，三版本平均时间无显著差异；G25OR在保持数学严格外包修复的同时成绩等价，因此作为最终方案。
6. PPO在当前实现中没有偏离A0先验，未独立产生速度收益；强化学习适合作为上层调度框架和消融对象，完成保证必须由确定性几何层提供。
